      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-muted">
            Cash outflows from your Starling import — {resolved.label}.
          </p>
        </div>
        <Link href="/dashboard/transactions" className={buttonClasses("secondary")}>
          Import Starling CSV
        </Link>
      </div>
      ...
      {!hasData ? (
        <div className="mt-8 rounded-2xl border border-dashed border-border bg-surface p-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted">
            Import a Starling CSV on the Transactions page to see spending here.
          </p>
          <Link href="/dashboard/transactions" className={`${buttonClasses("primary")} mt-4 inline-flex`}>
            Go to Transactions
          </Link>
        </div>
      ) : (
