...
export default async function RecurringInvoicesPage() {
    ...
    return (
      <div>
        ...
        <p className="mt-2 text-muted">
          Connect a database to manage recurring invoices.
        </p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-muted">
            Monthly retainers and schedules that create invoices automatically.
          </p>
        </div>
        {canWrite && (
          <Link
      ...
      <div className="mt-6">
        {rows.length === 0 ? (
          <p className="text-sm text-muted">No recurring schedules yet.</p>
        ) : (
          <Table>
            <THead>
              <TR>
            ...
            <TBody>
              {rows.map((row) => (
                <TR key={row.id}>
                  ...
                  <TD className="text-muted">{row.dayOfMonth}</TD>
                  <TD className="text-muted">{row.nextRunOn ?? "—"}</TD>
                  <TD className="capitalize text-muted">{row.onGenerate}</TD>
                  <TD>
                    <span
                      className={
                        row.enabled
